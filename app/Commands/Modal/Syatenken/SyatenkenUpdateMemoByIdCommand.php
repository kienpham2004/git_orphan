 ...
 * @author yhatsutori
 */
class SyatenkenUpdateMemoByIdCommand extends Command
{
    protected $id;
    ...
    public function handle(){
        ...
        // 値が空でない時に処理
        if( !empty( $setValues["tgc_alert_memo"] ) == True ){
            if( !empty( $tgcMObj->tgc_alert_memo ) == True ){
                $tgcMObj->tgc_alert_memo = $loginAccountObj->getUserName() . "さんからの伝達事項 " . date("Y/m/d") . "\n" . $setValues["tgc_alert_memo"] . "||" . $tgcMObj->tgc_alert_memo;
            }else{
                $tgcMObj->tgc_alert_memo = $loginAccountObj->getUserName() . "さんからの伝達事項 " . date("Y/m/d") . "\n" . $setValues["tgc_alert_memo"];
            }
