...
//            'tb_manage_info.mi_ctcshi_contact_date' => '活動実績日(試乗)',
            ...
            'tb_customer_umu.umu_csv_flg' => '車両有無',
    ...
    private function convert( $data ){
        ...
        foreach( $data as $key => $value ){
            ...
            $export[$key]['tgc_inspection_id'] = "車検";
            ...
            // 2022/03/29 update field check export
            if( isset($value['mi_dsya_syaken_reserve_date']) && date("Ym", strtotime( $value['tgc_syaken_next_date'] . '-12 month')) < date("Ym", strtotime($value['mi_dsya_syaken_reserve_date']) )){
                $export[$key]['mi_dsya_syaken_reserve_date'] = $value['mi_dsya_syaken_reserve_date'];
            }else{
                $export[$key]['mi_dsya_syaken_reserve_date'] = '';
            }

            $export[$key]['mi_rstc_get_out_date'] = $value['mi_rstc_get_out_date'];
//            2022/03/25 update field csv
//            $export[$key]['mi_rstc_delivered_date'] = $value['mi_rstc_delivered_date'];
            $export[$key]['mi_dsya_syaken_jisshi_date'] = $value['mi_dsya_syaken_jisshi_date'];

            // 2022/03/25 update field csv
            $export[$key]['mi_satei_max'] = $value['mi_satei_max'];
            $export[$key]['mi_ctcsho_contact_date'] = $value['mi_ctcsho_contact_date'];
            $export[$key]['mi_shijo_max'] = $value['mi_shijo_max'];
