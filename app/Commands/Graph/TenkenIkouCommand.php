...
use App\Models\Graph\TenkenGraphDB;
...
class CellClass{
    ...
    public function setVariables( $values ){
        ...
        // 2022/03/15 add field
        $this->mi_ctcshi_contact_date = $values->mi_ctcshi_contact_date;
        $this->mi_ctcsho_contact_date = $values->mi_ctcsho_contact_date;
        ...
        $this->tgc_inspection_ym = $values->tgc_inspection_ym;
 ...
 * @author yhatsutori
 */
class TenkenIkouCommand extends Command{
    ...
    public function handle() {
        ...
        // 担当者に紐づくデータを取得
        $allCustomerValues = TenkenGraphDB::getTenkenIkou( $this->search );
        // 担当者に紐づくデータを取得
        ...
        if( !empty($this->search->display_flg) && $this->search->display_flg == '1') {
            ...
            // 月別単位
            foreach ($userSortValues as &$userValues) {
                // セルの位置をずらす
        ...
        foreach ( $userObj->rows as $key => $monthObj ) {
            ...
            foreach ( $monthObj->cells as $cell ) {
                ...
                $statusFlg = (
                    ...
                    // 2022/03/21 add to right list
                    $cell->target_status == "104"
                );
