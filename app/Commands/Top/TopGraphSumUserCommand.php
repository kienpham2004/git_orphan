...
/**
 * スタッフ単位で車点検関連の集計を行うコマンド
 * @author yhatsutori
 */
class TopGraphSumUserCommand extends Command{
...
//                } else {
//                    // サマリーを取得する
//                    $graphData->{$name}[$ym] = collect( TopGraphDB::summaryUser( $search_tmp ) );
//                    // 拠点と担当者を表示するかどうかのフラグ
//                    $graphData->{$name}[$ym]->isStaffListType = true;
//
...
//                }
                ...
                if (!$graphData->{$name}[$ym]->isEmpty()){
                    // 拠点単位での合計を取得する
                    $graphData->{$name}[$ym]->total = TopGraphSumBaseCommand::getTotal($graphData->{$name}[$ym]);
                } else {
