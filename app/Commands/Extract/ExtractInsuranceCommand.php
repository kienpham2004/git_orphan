...
use App\Models\Views\VInsurance;
 ...
 *
 */
class ExtractInsuranceCommand extends Command implements ShouldBeQueued{
    ...
    public function handle(){
        ...
        //データ所得
        $this->findAndMergeInsurance( $from, $to, $storeTargets );


        // データを抽出
        foreach( $storeTargets as $target ){
            Insurance::merge( $target );
        }
     ...
     * @return [type]                [description]
     */
    private function findAndMergeInsurance( $from, $to, &$storeTargets ){
        // 対象データを取得
        $targetDatas = VInsurance::findInsurance( $from, $to );
