 ...
 * @author ohishi
 */
class ManageInfoNotFullCommand extends Command implements ShouldBeQueued {
     ...
     * @var string
     */
    private $tableName = 'tb_manage_info';
    ...
    public function handle()
    {
        ...
        if ( $this->checkingType == self::CHECKING_TODAY ) {
            ...
            $sql = "select * from 
                (
                    ...
                    where
                    (mi_inspection_id, mi_inspection_ym, mi_car_manage_number) in
                    (
                        select mi_inspection_id, mi_inspection_ym, mi_car_manage_number from {$this->tableName}
                        group by mi_inspection_id, mi_inspection_ym, mi_car_manage_number
                        having count(mi_inspection_id) > 1
                    )
                    order by id, mi_inspection_id, mi_inspection_ym, mi_car_manage_number
                ) as x";
        }
        ...
        foreach ($items as $index => $item) {
            ...
            // 重複データの絞り込み条件を作成
            $criteria = "({$this->tablePrefix}_delete_target IS NULL) AND ";
