 ...
 * @author ohishi
 */
class ResultNotFullCommand extends Command implements ShouldBeQueued {
        ...
        if ( $this->checkingType == self::CHECKING_TODAY ) {
            ...
            $sql = "select * from 
                (
                    ...
                    where
                    (rst_manage_number, rst_detail, rst_syaken_next_date) in
                    (
                        select rst_manage_number, rst_detail, rst_syaken_next_date from {$this->tableName}
                        group by rst_manage_number, rst_detail, rst_syaken_next_date
                        having count(rst_manage_number) > 1
                    )
                    order by id, rst_manage_number, rst_detail, rst_syaken_next_date
                ) as x";
        }
