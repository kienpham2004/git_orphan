...
class VResultAnkai extends AbstractModel {
    // テーブル名
    protected $table = 'v_tenken_ansin_kaiteki_result';
    ...
    public function scopeJoinCustomer( $query ) {
        $query->leftJoin(
            'tb_customer',
            function( $join ){
                $join->on( 'v_tenken_ansin_kaiteki_result.rst_manage_number', '=', 'tb_customer.car_manage_number' )
    ...
    public static function findTarget( $from, $to ) {
        // 他のテーブルとJOIN
        $builderObj = VResultAnkai::joinCustomer();
