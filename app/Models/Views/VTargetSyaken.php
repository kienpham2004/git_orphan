...
class VTargetSyaken extends AbstractModel {
    // テーブル名
    protected $table = 'v_syaken';
    ...
    public function scopeJoinCustomer( $query ) {
        $query->leftJoin(
            'tb_customer',
            function( $join ){
                $join->on( 'v_syaken.car_manage_number', '=', 'tb_customer.car_manage_number' )
    ...
    public static function findTarget( $from, $to ) {
        // 他のテーブルとJOIN
        $builderObj = VTargetSyaken::joinCustomer();
